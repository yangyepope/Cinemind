...
import streamlit as st  # 导入 streamlit 以利用 cache_resource 实现单例
...
def init_vector_store() -> Chroma:
    """
    初始化并返回 Chroma 向量存储对象（全局单例）。

    使用 @st.cache_resource 装饰器确保整个应用生命周期内只初始化一次，
    避免 kb_service 和 rag_chain 分别创建独立实例导致的资源浪费与不一致。
    同时有效规避 Streamlit 在页面刷新时导致的 Event loop 异常。
    ...
    vector_store: Chroma = Chroma(
        ...
        collection_name="cinemind_rag_knowledge"
    )
