...
## 2. `RunnableWithMessageHistory` 底层执行流

这是 LangChain 实现“自动记忆”的核心装饰器。它的执行遵循“预处理 -> 核心运行 -> 后处理”的严谨顺序。
...
### 2.1 核心执行逻辑流程图 (Logic Flowchart)

为了更直观地展示逻辑判定与分支，我们将其转化为逻辑流程图：

```mermaid
graph TD
    Start([用户调用 invoke]) --> Merge[_merge_configs: 合并配置与提取 ID]
    Merge --> Factory[调用 get_session_history 工厂函数]
    Factory --> Instance[创建 History 实例并挂载到 config]
    Instance --> Enter[_enter_history: 读取历史阶段]
    Enter --> LoadDB[(从存储介质读取 Messages)]
    LoadDB --> Combine[历史消息 + 新输入打包]
    Combine --> LLM{LLM 核心推理}
    LLM --> Exit[_exit_history: 保存与清理阶段]
    Exit --> SaveDB[(批量保存 add_messages)]
    SaveDB --> End([返回结果给用户])
